...
public class YoutubeLayout extends ViewGroup {
	...
	boolean smoothSlideTo(float slideOffset) {
		...
		if (mDragHelper
				.smoothSlideViewTo(mHeaderView, mHeaderView.getLeft(), y)) {// 设定子View滑动
	...
	private class DragHelperCallback extends ViewDragHelper.Callback {
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			...
			if (yvel > 0 || (yvel == 0 && mDragOffset > 0.5f)) {// Y
																// 轴速度大于o，或者是滑动超过一半
				top += mDragRange;
			}
			mDragHelper.settleCapturedViewAt(releasedChild.getLeft(), top);// 設定位置
			invalidate();
		}
	...
	public void computeScroll() {
		...
		if (mDragHelper.continueSettling(true)) {
			ViewCompat.postInvalidateOnAnimation(this);// 接著显示下一帧
		}
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (action == MotionEvent.ACTION_CANCEL
				|| action == MotionEvent.ACTION_UP) {// 子View不接受OnTouch事件
			mDragHelper.cancel();
			return false;
		}
	...
	private boolean isViewHit(View view, int x, int y) {
		...
		view.getLocationOnScreen(viewLocation);// 整個屏幕绝对坐标
		int[] parentLocation = new int[2];
		this.getLocationOnScreen(parentLocation);
		int screenX = parentLocation[0] + x;// 相对于屏幕坐标
		int screenY = parentLocation[1] + y;
		return screenX >= viewLocation[0]
				&& screenX < viewLocation[0] + view.getWidth()
				&& screenY >= viewLocation[1]
				&& screenY < viewLocation[1] + view.getHeight();// 在headerView内部
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		// System.out.println("dragLyouytheight:: " + getHeight());
		mDragRange = getHeight() - mHeaderView.getHeight();// 498
		// System.out.println("mHeaderView.getHeight(); "
		// + mHeaderView.getHeight() + "   mDragRange: " + mDragRange);
